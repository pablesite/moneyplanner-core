...
def expense_entry_applies_to_fiscal_year(*, entry: AnnualExpenseEntry, fiscal_year: int) -> bool:
    if entry.is_system_generated and (entry.source_asset_id or entry.source_liability_id):
    ...
    return (
        ...
        or entry.term_end_year is None
        or entry.term_end_year >= fiscal_year
    )


def effective_annual_expense_entries(*, user, fiscal_year: int):
    """Budget rows effective in a year, matching the recurrence contract of its API.

    Asset/liability mirrors are explicit yearly slices. Manual rows and plan-managed
    rows without a linked source retain their recurrence profile across fiscal years.
    """
    autogenerated_exact_year = Q(is_system_generated=True) & (
        Q(source_liability_id__isnull=False) | Q(source_asset_id__isnull=False)
    )
    manual_or_non_linked = Q(is_system_generated=False) | (
        Q(source_liability_id__isnull=True) & Q(source_asset_id__isnull=True)
    )
    return AnnualExpenseEntry.objects.filter(user=user).filter(
        (autogenerated_exact_year & Q(fiscal_year=fiscal_year))
        | (
            manual_or_non_linked
            & (
                Q(
                ...
                | Q(
                    ...
                    fiscal_year__lte=fiscal_year,
                )
                | Q(
                    time_profile=AnnualExpenseEntry.TimeProfile.TERM_RECURRENT,
                    fiscal_year__lte=fiscal_year,
                )
                & (Q(term_end_year__isnull=True) | Q(term_end_year__gte=fiscal_year))
            )
        )
    )
...
def planned_expense_monthly_distribution(
    *, entry: AnnualExpenseEntry, fiscal_year: int
    ...
    if entry.time_profile == AnnualExpenseEntry.TimeProfile.TERM_RECURRENT and (
        entry.term_end_year == fiscal_year
        or (entry.term_end_year is None and entry.fiscal_year == fiscal_year)
    ):
...
def effective_annual_income_entries(*, user, fiscal_year: int):
    return AnnualIncomeEntry.objects.filter(user=user).filter(
        Q(time_profile=AnnualIncomeEntry.TimeProfile.ONE_OFF, fiscal_year=fiscal_year)
        | Q(
