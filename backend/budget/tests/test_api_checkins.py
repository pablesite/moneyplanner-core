...
class AnnualExpenseApiCheckinsTests(APITestCase):
    ...
    def test_expense_monthly_summary_exposes_unbudgeted_execution_breakdown_contract(self):
        ...
        tx_budgeted = LedgerTransaction.objects.create(
            ...
            description="Compra presupuesto",
            status=LedgerTransaction.Status.POSTED,
        )
        LedgerEntry.objects.create(
        ...
        tx_unbudgeted = LedgerTransaction.objects.create(
            ...
            description="Compra no presupuestada",
            status=LedgerTransaction.Status.POSTED,
        )
        LedgerEntry.objects.create(
        ...
        categories = {
            row["category"]: row
            for row in response.data["expense_execution_breakdown"]["categories"]
        }
        subcategories = {
            row["subcategory"]: row for row in categories["consumption_expenses"]["subcategories"]
        }
