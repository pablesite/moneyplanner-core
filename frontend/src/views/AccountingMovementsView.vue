  ...
  <div class="container ui-page-shell">
    ...
    >
      ...
      >
        ...
        <!-- Revaluation: investment account + new total value -->
        <template v-if="quickEntryForm.movement_type === 'revaluation'">
          <div class="ui-accounting-form-grid ui-accounting-form-grid-wide">
            <select v-model="quickEntryForm.account_id" class="select" required>
              <option :value="null">Cuenta de inversion</option>
              <option
                v-for="account in revaluationAccountOptions"
                :key="account.id"
                :value="account.id"
              >
            ...
            <input
              v-model="quickEntryForm.revaluation_new_value"
              class="input"
              inputmode="decimal"
          ...
          <p class="ui-accounting-inline-note">
            <template v-if="revaluationCurrentBalance != null">
              Saldo actual:
              <strong>{{ revaluationCurrentBalance.toFixed(2) }}</strong>.
              <template v-if="revaluationDelta != null">
                Revalorizacion:
                <strong
                  :class="
                    revaluationDelta > 0
                  ...
                  "
                >
                  {{ revaluationDelta >= 0 ? '+' : '' }}{{ revaluationDelta.toFixed(2) }}
                </strong>
              </template>
            </template>
            <template v-else>
              Selecciona la cuenta de inversion para ver el saldo actual.
            </template>
          </p>
        </template>

        <!-- Standard types: liquidity account + amount + optional counterparty -->
        <div
          v-else
          class="ui-accounting-form-grid"
          :class="
