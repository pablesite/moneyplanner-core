...
// Guardamos los últimos datos del timeline global (sin filtro de categoría).
// Cuando se selecciona una categoría, store.timeline se sobreescribe con datos
...
watch(
  ...
  ([rows, cat]) => {
    if (cat === null) {
      ...
      globalPrevMonthSameDay.value = store.timeline?.prev_month_same_day ?? null;
    }
  },
  { immediate: true },
);
...
const monthlyDelta = computed<{
  ...
  const sameDayData = globalPrevMonthSameDay.value;
