...
type Props = {
  ...
  totalBase?: string;
  onUpdate: (id: number, payload: Partial<Item> & { financed_asset_id?: number | null }) => Promise<void>;
...
const amountError = computed(() => {
  ...
  if ((normalizeAmountInput(draft.value.amount).match(/\./g) || []).length > 1) return "Importe invalido";
...
function subcategoryPercent(category: string, subcategory: string | null, items: Item[], categoryItems: Item[]) {
  ...
  <div class="card">
    <div class="card-header">
      ...
      <div class="card-header-right">
        ...
        <button v-if="onAdd" class="btn btn-primary btn-sm add-icon-only" type="button" @click="onAdd" aria-label="Anadir">
    ...
    <div v-else style="display:grid; gap:16px;">
      <section v-for="g in grouped" :key="g.category" class="cat-block" :class="categoryClass(g.category)">
        <div class="cat-header">
          ...
          <div class="cat-right">
            ...
            >
              <span class="icon" aria-hidden="true" v-if="expandedCats.has(g.category)">&#9662;</span>
              <span class="icon" aria-hidden="true" v-else>&#9656;</span>
            </button>
          </div>
        </div>

        <div v-if="shouldShowGroupDetails(g)" :class="g.hasSubgroups ? 'subcat-list' : ''">
          <div v-for="(sg, sgIndex) in g.subgroups" :key="subgroupKey(g, sg, sgIndex)" :class="g.hasSubgroups ? 'subcat-block' : ''">
                ...
                <div v-if="editingId !== it.id" class="item-row">
                  <div class="item-main">
                    ...
                    <div v-if="isLiabilitiesList && it.financed_asset_ref" class="item-submeta">
                      <span class="financed-text">· Financia: {{ financedAssetName(it.financed_asset_ref) }}</span>
                    </div>
                  </div>
                  <div class="item-amount">
                    {{ formatAmount(String(it.amount), { currency: it.currency }) }} {{ it.currency }}
                  </div>
                  <div class="actions">
                    <button
