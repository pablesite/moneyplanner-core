...
- `476` - `Cuenta Naranja` (activo de liquidez; cierre final completado)
- `18` - `MyInvestor (Compartido)` (activo de liquidez; cierre final completado)
...
- Ajuste adicional de categorizacion en `ING` (id=5) aplicado en esta sesion sobre gastos en `consumption_expenses/other_consumption_expenses` (vinculados a transacciones de ING): `Alimentacion o similar` -> `living_expenses` (3), `iCloud o similar` -> `education_growth` (59), `Aparcamiento o similar` -> `transport_mobility` (9).
