...
- `Monedero compartido` (id=16) en revision activa. Detectado y añadido manualmente movimiento faltante de transferencia por `600 EUR` con fecha `2020-11-26`. Retomar la auditoria desde este punto en la siguiente sesion para verificar que no falten mas transferencias/movimientos relevantes.
...
1. Continuar revision de `16` (Monedero compartido) desde el ajuste manual del `2020-11-26` (`600 EUR`) y comprobar integridad de transferencias/movimientos relevantes.
2. Cripto satelite pendiente: `445` (DT Bots Cripto).
3. Depositos dentro de liquidez: `5` (ING), `21` (Santander), `16`/`17`/`15` (Monederos).
