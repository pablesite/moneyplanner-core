...
| 5 | **Miembros del plan** | Se extiende `FamilyMember` (migración aditiva en `memberships`) con campos opcionales: `birth_date`, `employment_income_end_date`, `pension_start_date`, `estimated_monthly_pension_today_eur`, `other_future_income_today_eur`. `FinancialPlan` selecciona adultos participantes vía M2M (máx. 2 en MVP). No se crea `PlanMember`. Con fecha de nacimiento, el motor deriva jubilación a los 67 años; las fechas persistidas solo son fallback compatible si falta nacimiento. |
...
| 7 | **Aportación mensual (precedencia)** | Planificada = `InvestmentContributionInterval` + líneas de presupuesto efectivas con `cashflow_role ∈ {savings, investment}`. Es una asignación del flujo libre, nunca una entrada: el motor la limita a la caja disponible y reparte cualquier recorte proporcionalmente entre ahorro e inversión. Real = ledger (`quick_entry_kind=investment`) / `InvestmentAssetEvent`. El suavizado (FR-PROJ-008) usa la aportación planificada, no la del último mes. |
