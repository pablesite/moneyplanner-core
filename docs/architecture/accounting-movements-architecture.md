...
14. Known gap, not addressed: a divested position keeps reporting its last stale valuation instead of zero, so periods long enough to include archived positions overstate the total (3215.26 EUR over 9 positions on current production data). `closed_on` cannot drive the fix because bootstrap backfills it with `asset.updated_at`, which records the migration date and not the divestment; the reliable signal is a zero posted balance.
15. Signals only cover changes made through the ORM. Data that reaches the database underneath it (a restore, a bulk load) fires nothing, so `POST /api/portfolio/positions/resync-valuations/` is the explicit way out of that drift; `/cartera` exposes it as "Actualizar desde contabilidad".
