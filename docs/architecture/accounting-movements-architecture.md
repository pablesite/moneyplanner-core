...
16. Signals only cover changes made through the ORM. Data that reaches the database underneath it (a restore, a bulk load) fires nothing, so `POST /api/portfolio/positions/resync-valuations/` is the explicit way out of that drift; `/cartera` exposes it as "Actualizar desde contabilidad".
