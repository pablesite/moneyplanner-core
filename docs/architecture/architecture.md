...
7. Read APIs are `/api/portfolio/overview/`, `/api/portfolio/positions/performance/`, `/api/portfolio/timeline/`, `/api/portfolio/performance/` and `/api/portfolio/quality/`. They accept `date_from`, `date_to` and optional user-owned `member_id`. Timeline range is capped at 20 years and bulk-loads positions, values, ledger prefixes, flows, FX and ownership with a query count independent of position count. Its performance context keeps chronological FX date indexes beside the shared reverse-chronological rate cache, so a fresher close is resolved by binary search.
8. `GET /api/portfolio/workspace/` accepts `include_timeline=false`, `include_position_details=false` and `include_flows=false`. The first preserves the timeline envelope with no points and builds only the requested-period context; the second returns just the position values/classification needed for Resumen and declares that reduced row shape with `position_details=false`; the third keeps all aggregate return metrics but omits the technical flow rows. All default to `true` for API compatibility; SaaS requests only the data each view renders.
9. No persistent cache is currently used. Read models are deterministic and rebuildable; this avoids incomplete invalidation until profiling justifies caching. Any future cache must be invalidated by flow, price/value, FX, CPI or ownership changes and can never become a source of truth.
...
Class composition is resolved in `portfolio/composition.py`: the latest holdings snapshot dated on or before the read takes precedence over the undated manual class split, then the effective position class. Allocation, exposure and workspace composition consume this same resolver. A partial snapshot retains its missing fraction as `unclassified`; it does not borrow the missing percentage from the fallback or shrink the position value. Invalid overallocated snapshots are treated as entirely unclassified. Allocation keeps full `by_position` values and adds `class_breakdown` slices (value and share of the allocation total), `composition_source`, `composition_observed_on` and `class_covered_percent`. A product target still belongs to its declared policy class: exposure-aware mixed-product purchase sizing is pending in decision-system task 4, not implied by this read correction.
...
3. Tactical liquidity is a policy line with its own band, not the remainder of the operation. Linked `ContainerCashAccount` balances are the liquidity that line reads against, attributed to a scope through the `OwnershipLink` of the cash asset itself. Allocation, exposure (when scoped) and contribution solving use this same dated ownership perimeter. A source already inside that perimeter is a relocation and is never added again; an external source is new money. The solver reports the existing cash, tactical reserve, money parked for a position minimum and the operating remainder separately. An external tactical reserve creates a transfer line to a linked cash account in the same scope; without that destination the proposal is blocked.
4. `PositionAllocationRule` carries the constraints that make a proposal executable: exclusion, minimum contribution, rounding step, operation cost and whether a fee-free recurring plan applies. `ContributionCommitment` carries what must reach a position regardless of drift — a monthly floor that keeps a broker perk, or an annual quota, which is a ceiling too, since contributing above a deductible cap does not deduct. `PortfolioPosition.tax_transferable` marks what can be moved without a tax toll, so the solver prefers the free pocket when two options tie. A commitment hangs from a position or from a **container**: the platform's monthly minimum is the platform's, not any one product's — MyInvestor asks for 300 € a month and does not care whether they land in the pension plan or the roboadvisor. Serving it splits the amount across that container's positions in proportion to the annual quota each one has left, and an annual quota is claimed **paced over the months remaining** rather than in full: emptying it in January leaves the following months with nothing to contribute there, which with a monthly platform floor in play means losing the perk. Only external money counts towards a floor, because buying inside a platform with cash already parked there is not new money arriving. A commitment also declares `breach_cost`, what breaking it costs per year: a commitment is not worth its amount but what is lost by missing it — dropping the recurring plan that keeps a remunerated account can cost the yield of the whole bank balance. When the contribution cannot cover every commitment, that cost decides who is served first — but **only between unequal costs**: commitments that cost the same to break share what there is in proportion to what each claims, because serving one whole and leaving the other at zero is a decision nobody took. What is left uncovered is reported instead of quietly disappearing.
...
7. A contribution proposal is evidence-gated. Missing or stale valuations, missing FX, incomplete class coverage, stale holdings, invalid ownership, negative/duplicated cash and an invalid funding source return the dated issue and a corrective action instead of omitting value or treating it as zero. A carrying value remains a visible warning. Confirmation rechecks blockers at the basket booking date and rejects an origin whose internal/external perimeter changed since the preview.
8. A reviewed contribution carries a deterministic `review_token` over its amount, date, ownership, policy, cash/data evidence, destinations and projected impact. Saving through the API requires that token and recomputes it, so a change does not silently save a different allocation. Confirmation recomputes it again. The proposal publishes `impact`: total and each class's value, weight and band before and after the planned movements. The impact is an execution preview, never a claim that accounting has already changed.
...
## Investment Portfolio Exposure and Concentration

1. The asset class answers what makes a position rise or fall. Exposure answers the next question, which is the one that decides whether you are diversified: how much of your portfolio depends on North America, on technology, on one vehicle. `PositionExposure` declares, per position and dimension (geography, sector, vehicle), the weight of each bucket **within that position**, with the reference date of the factsheet it was copied from. Vehicle distinguishes property crowdfunding from consumer-loan crowdlending, since they have different liquidity, credit and collateral risks.
2. The weights are entered by hand on purpose. No provider gives a fund's internal split from its ISIN — that is a different dataset from pricing, sold under licence — and the numbers are a dozen per position that move slowly. What the system contributes is not capturing them but adding them up and **declaring what it does not know**.
3. The aggregate splits over the **declared** value, not the total: computing over the total would make every slice look smaller than it is and the parts would not add to a hundred. Coverage travels beside it (`ready` at 80% or more, `partial` above zero, `insufficient` at zero) along with the oldest factsheet date, because an exposure that has aged silently misleads more than none at all. Sector also publishes coverage over its applicable universe: equity from dated holdings first, class breakdown second and the position class last. Its total-portfolio coverage remains visible, but gold, private debt and crypto do not make a complete equity sector factsheet look incomplete.
4. Concentration reports the weight of the five largest positions and the portfolio's **effective number of equal positions** (`1/HHI`). The 0–1 index is published too, normalized against the perfect split of those same positions; the fixed 0.2 denominator used by financial-health saturates at 100% for any portfolio with more than a handful of positions, so it says nothing there.
5. Overlap compares two positions' declared splits bucket by bucket and reports the shared area, plus how much money is exposed twice to the same thing. It is overlap of **exposure**, not of underlying securities — which is precisely the question asked when looking at a global index fund and a roboadvisor's US sleeve. Pairs below 25% are not published: two global products always share something, and listing every pair would turn a signal into noise.
6. `PositionHolding` adds the verifiable layer: a dated factsheet snapshot of underlying name, optional stable identifier (ISIN/ticker), class, geography, sector and position weight. At any requested date the newest snapshot not newer than that date wins. Its rows derive class, geography and sector for that position; when none exist, the manual class breakdown and declared exposure remain the fallback. A missing fraction remains `unclassified` in the class composition, with its value retained and coverage reduced. Class row percentages use the whole valued position universe (`percent_basis: positions_total`), including that unknown fraction; geography, sector and vehicle continue to use their declared bases. Container cash remains outside the exposure endpoint in this delivery; harmonizing that perimeter is decision-system task 2. Matching stable identifiers exposes exact shared-underlying overlap; matching names is explicitly only a fallback for factsheets without identifiers.

## Investment Portfolio Benchmark, Risk and Decision Log

1. The primary benchmark is **not an index**: it is the portfolio's own written policy. Each month takes the strategy version in force **on that date** —not today's, which would judge March by what was decided in July— and composes what its target weights would have returned given how the user's own classes actually behaved. Compared against what the portfolio really did, the excess answers one question: **did drifting from the plan help**. An index answers a different one —whether picking those products paid off— which is why it stays secondary and optional (`AllocationStrategy.benchmark_instrument`, computed only when its prices exist in the portfolio's base currency).
2. Cash is left out of **both** sides. The portfolio series is the scope's positions, and container cash belongs to no class, so including it only on the benchmark side would compare two different things. The remaining class weights are renormalized and the payload says so.
3. Conventions, fixed before any formula because they are the difference between a comparable figure and an invented one: **month-end calendar** (the partial stubs at either edge of the chosen period are excluded — a nine-day stub treated as a month deforms anything later annualized), **monthly frequency** annualized by 12 for returns and √12 for deviation, and everything computed on the **chained return series**, never on value: a contribution raises value without being a recovery, and measuring drawdown on value would turn every contribution into an improvement.
4. The risk-free rate is a configurable service (`PORTFOLIO_RISK_FREE_RATE`, annual, decimal; 2% default) and travels published beside every figure that uses it — the only thing that lets one Sharpe be compared with another. Sharpe is computed on the monthly excess, not by subtracting two already-annualized figures, which would mix two annualizations.
5. Coverage rules. Volatility, annualized return and Sharpe need **12 monthly observations**; max drawdown, which reads a path rather than estimating a parameter, needs three. Metrics are measured over the **longest uninterrupted stretch** of months, and the payload reports the window used, the months in the period and which ones are missing: one month without a valuation out of eighteen cannot silence the whole screen, but chaining January to March would treat two months as one. A gap is declared before a short count — they are different problems with different fixes. A flat series has no Sharpe rather than an infinite one. The benchmark payload also publishes the policy's compounded annualized return over the longest complete benchmark run, so its historical chart and headline never mix an accumulated figure with an annualized one calculated over another window.
6. The benchmark read also publishes **rolling twelve-month windows**: each point compounds exactly the same twelve complete months for portfolio and written policy, so it compares equivalent horizons instead of making the first years look artificially quiet. Historical VaR at 95% is a one-month loss percentage from the empirical tail and requires 24 complete months; it is not a forecast of the worst possible loss. When a policy declares a same-currency secondary index with complete monthly closes, beta and Pearson correlation use the longest contiguous common monthly run and require 12 observations. Position correlation and risk contribution use the same uninterrupted monthly window, only positions with a complete return series, and their **current closing weights**. Contribution means share of the modelled annualized volatility, not historical P&L attribution; both payloads disclose the included positions and value coverage so an illiquid or recently added slice is never silently represented.
7. The **decision log** (`GET /api/portfolio/decisions/`) records what the system proposed (the contribution basket), what the user did with it (confirmed whole, line by line, or discarded) and the worst class drift at the decision date and now. It is observed history, not a backtest: comparing against pure DCA or buy & hold on a few real bets and a short history would measure noise and call it system, and for the illiquid part it is not even computable. No new model — the basket already holds the proposal, its lines hold what was executed and the ledger holds what was booked; a second copy would only be a second version of the truth.

## Investment Portfolio Product Integration

1. `GET /api/portfolio/alerts/` is a deterministic read model, scoped to the authenticated user's single portfolio. It groups actionable quality, structure and execution facts: missing/stale valuations, ownership and FX gaps, policy absence or drift, concentration, incomplete declared exposure and draft contribution baskets. Alerts are rebuilt on every read and are never persisted or sent outside Cartera in this MVP.
2. Structural thresholds are settings-backed where they are global (currently the concentration threshold); allocation bands remain the user's versioned strategy. Each alert contains a typed internal action, not an investment instruction, so SaaS can take the user to the valuation queue, setup, allocation, exposure, basket or Patrimonio without parsing copy.
3. Mi Plan still receives its asset inventory from Patrimonio. For an active `PortfolioPosition`, it replaces that linked Asset's effective amount only when Cartera resolves a `fresh` or `at_cost` close in the requested base currency. A stale, missing or non-convertible close leaves Patrimonio's value in place and reports partial portfolio coverage. This makes productive capital use the best available valuation once, never the sum of two inventories.
4. The integration adds no models or mutable alert state. Existing database backup/restore remains the recovery boundary; portable JSON has no new coverage obligation because no new portable data is created by alerts or the Plan adapter. Fiscal FIFO, broker credentials/connectors, automatic holdings look-through and advanced risk remain intentionally outside this delivery.
