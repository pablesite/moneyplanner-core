...
8. `GET /api/portfolio/workspace/` accepts `include_timeline=false` and `include_position_details=false`. The first preserves the timeline envelope with no points and builds only the requested-period context; the second returns just the position values/classification needed for Resumen and declares that reduced row shape with `position_details=false`. Both default to `true` for API compatibility; Evolución opts into the timeline and Posiciones into its full per-position metrics.
