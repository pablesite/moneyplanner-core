...
7. Recurring `savings/investment` budget rows describe the intended use of cash but do not create cash themselves. Accumulation is governed by the complete free operating surplus, so the projection cannot omit unassigned savings or invest more than income actually supports.
7b. Active `one_off` budget entries **not** governed by a decision feed the projection as year-specific flows in their `fiscal_year` across the whole horizon (`services_inputs.one_off_flows`): income and `savings/investment` add capital, `asset_purchase` moves productive→non-productive (cash becomes an asset), and `tax_fee/transfer/other` are pure outflows (`apply_initial_outflow`). Excluded: past years (already in current capital), `is_system_generated` expenses (owned by Patrimonio), `asset_sale` income (modeled via decisions) and entries whose `event_group` starts with `plan_event:`. For the current year only not-yet-occurred entries count (`target_month` ≥ current month, or no month), so already-spent/received amounts are not double-counted.
7c. **Cash-flow reconciliation and allocation**: in each accumulation year (`year < target_year`), including the remaining months of the current year, `free_surplus = income − operating_expenses − active_temporary_commitments − decision_debt_service` (`services_projection.free_operating_surplus`). Positive surplus first recovers any `financing_gap` and honors explicit event contribution destinations; the rest is split by `security_contribution_rate` (25% by default) into Security until it reaches `annual_operating_expense × security_target_expense_years` (two inflation-adjusted years by default), with all excess going to Productive. A negative surplus consumes Security first, then Productive, and any uncovered amount remains as a recoverable negative `financing_gap`. Temporary commitments expire by `term_end_year`; a user's existing debt service is already represented by its budget line, while new decision debt uses `decision_debt_service_for_year`, so neither is paid "for free" nor counted twice. Known gap: `FoundationService.committed_surplus` reads budget lines only, so a decision's new debt is not yet reflected in the cash-flow *diagnosis* (only in the projection).
7d. **Projected balance composition**: every trajectory row exposes gross `liquidity_assets`, `investment_assets`, `real_estate_assets`, `furnishings_assets`, `other_assets` and their `total_assets`. Housing/renovation decisions move Real estate; vehicle decisions move Furnishings; security allocations move Liquidity and productive allocations move Investments. Real estate uses `non_productive_appreciation_rate`; furnishings and vehicles use the explicit `furnishings_depreciation_rate` (12% default). Displayed amounts are rounded by category before totaling, and `net_worth = total_assets − liabilities` exactly.
...
4. The current-year row starts after the current net-worth snapshot: structural cash flow and temporary commitments use only the remaining budget months, and new debt service is prorated from `transaction_month`. Decision inflows and outflows in the same month are netted before touching capital buckets, so a linked home sale funds its replacement purchase without artificially draining Security and returning the same cash as Productive. Annual standalone one-off inflows/outflows are likewise netted before bucket allocation. If the resulting cash need cannot be funded, the remainder is exposed as negative `financing_gap`, included in projected liabilities and repaid by future free cash before new contributions.
...
5. Recommendation preview calculates the current and simulated projections without creating a scenario, snapshot or budget row. Simulation creates a draft `Scenario` linked through `source_recommendation`; the recommendation becomes accepted only when that scenario is incorporated.
