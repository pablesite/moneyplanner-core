...
7. Read APIs are `/api/portfolio/overview/`, `/api/portfolio/positions/performance/`, `/api/portfolio/timeline/`, `/api/portfolio/performance/` and `/api/portfolio/quality/`. They accept `date_from`, `date_to` and optional user-owned `member_id`. Timeline range is capped at 20 years and bulk-loads positions, values, ledger prefixes, flows, FX and ownership with a query count independent of position count. The shared reverse-chronological FX cache keeps a chronological date index, so ordinary conversion and a fresher close are both resolved by binary search.
