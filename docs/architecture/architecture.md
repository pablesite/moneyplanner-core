...
8. `GET /api/portfolio/workspace/` accepts `include_timeline=false`. It preserves the timeline envelope with no points and builds only the requested-period context, because Resumen, Posiciones, Asignación and Diversificación do not render the historical series. The default remains `true` for API compatibility; Evolución opts in explicitly.
