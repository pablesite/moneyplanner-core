...
7c. **Cash-flow reconciliation**: in each accumulation year (`year < target_year`) the *effective* contribution is capped by the real free surplus, not the budgeted amount: `free_surplus = income − operating_expenses − active_temporary_commitments − decision_debt_service` and `effective_contribution = min(planned, max(0, free_surplus))` (`services_projection.free_operating_surplus`). Temporary commitments (`cashflow_role=temporary_commitment`) are nominal and expire by `term_end_year`; `decision_debt_service` is the annual French-amortized installment of **new** decision debts only (`decision_debt_service_for_year`), active while `start_year ≤ year ≤ start_year + term − 1` — a user's *existing* debt already appears as a temporary-commitment budget line, so it is never double-counted. If `free_surplus < 0` the deficit consumes capital (`apply_initial_outflow`: security first, then productive). This is why a new mortgage no longer amortizes "for free": its installment competes with contributions for the same surplus. The current/base year (`offset == 0`) is a snapshot and is not reconciled. Known gap: `FoundationService.committed_surplus` reads budget lines only, so a decision's new debt is not yet reflected in the cash-flow *diagnosis* (only in the projection).
