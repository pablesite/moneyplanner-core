...
| Limpieza legacy residual | Media | 🔄 | Inventario vivo documentado en `roadmap/product-roadmap.md`. Avance: `portable-data` y `budget/annual-entries` extraídos de `data-input`; quedan taxonomías/filtros legacy, fallback Budget/check-ins, alias `investment_purchase`, facade `net_worth.services`, campos legacy de aportaciones, compat portable y `compat.*`. |
