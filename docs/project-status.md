...
| Monthly Close | ✅ | Integrated with budget and accounting. Automatic dual mode remains unchanged for profiles without settlement. Optional settlement inputs now include a disabled-by-default profile, participating account roles, structured budget ownership/routes, period readiness, activation baseline and physical-wallet normalization. The economic solver remains Phase 3. |
