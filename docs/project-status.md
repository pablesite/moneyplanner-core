...
| Monthly Close | ✅ | Integrated with budget and accounting. Automatic dual mode remains unchanged for profiles without settlement. The opt-in ownership settlement resolves per-member opening/income/expense/requirement/excess balances, cross-payment compensations, recurrent reserves and fixed/dynamic destinations, then returns reconciled routes without creating ledger movements. SaaS v1 presents that frozen/provisional contract and deep-links manual transfers; finalize freezes an immutable snapshot even when quality is `not_ready`. Phase 6 will materialize routes idempotently. |
