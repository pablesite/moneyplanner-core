...
| Limpieza legacy residual | Media | 🔄 | Inventario vivo documentado en `roadmap/product-roadmap.md`. Primer avance: `portable-data` extraído de `data-input`; quedan `data-input` annual entries/taxonomías, fallback Budget/check-ins, alias `investment_purchase`, facade `net_worth.services`, campos legacy de aportaciones, compat portable y `compat.*`. |
