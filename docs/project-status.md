...
| Investment Portfolio — pre-opening dates count zero | ✅ | A position resolves to zero, not unknown, on dates at or before `opened_on` with nothing recorded, so the portfolio total and TWR stop being withheld whenever a position opened mid-period. Pending: a divested position still reports its last stale valuation instead of zero, which overstates long periods; `closed_on` is unusable for it because bootstrap backfills it with the migration date. |
