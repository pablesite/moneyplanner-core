...
| Financial Plan — engine correctness | ✅ | Phase 7 complete: current-FY inputs, structural-only income, aggregate labour cut-off, exhaustive expense buckets and explicit data-quality factors. Current-year projection now starts after the live snapshot, uses remaining monthly budget slices, prorates decision debt service and carries unfunded cash as a recoverable `financing_gap`. Historical snapshots remain immutable. |
