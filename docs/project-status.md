...
| Financial Plan — engine correctness | ✅ | Phase 7 complete: current-FY inputs, structural-only income, aggregate labour cut-off, exhaustive expense buckets and explicit data-quality factors. Current-year projection starts after the live snapshot, uses remaining monthly budget slices, prorates decision debt service and carries unfunded cash as a recoverable `financing_gap`. Same-month decision cash and annual standalone one-offs are netted before bucket allocation, preventing a linked sale/purchase from turning existing Security into Productive merely because of event ordering. Disposing a financed asset removes its net classified value, so its associated mortgage is not subtracted twice. Historical snapshots remain immutable. |
