...
| Investment Portfolio — asset class taxonomy reworked | ✅ | Ten classes replace the seven generic ones: renta fija, renta variable, inmobiliario, capital privado, activos refugio, materias primas, inversiones alternativas, trading, liquidez para oportunidades y otros. Migration `0011` remaps every retired value with a tested reverse (`cash`→`opportunity_cash`, `crypto`→`safe_haven`, `real_assets`→`real_estate`, `mixed`→`other`). Only eight categorical hues clear the palette checker, so two low-traffic classes share the neutral token and the composition donut folds past eight into "Otras clases" rather than inventing colours nobody can tell apart. |
