...
| Limpieza legacy residual | Media | ⚪ | Inventario vivo documentado en `roadmap/product-roadmap.md`: `data-input` como dominio compartido, fallback Budget/check-ins, alias `investment_purchase`, facade `net_worth.services`, campos legacy de aportaciones, compat portable y `compat.*`. |
