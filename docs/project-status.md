...
| Monthly Close | ✅ | Integrated with budget and accounting. Automatic dual mode remains unchanged for profiles without settlement. The opt-in ownership settlement now resolves per-member economic balances, cross-payment compensations, next-month recurrent reserves and fixed/dynamic allocation destinations, then returns reconciled transfer routes without creating ledger movements. Finalize freezes an immutable snapshot even when quality is `not_ready`; reopen recalculates explicitly. Phase 3 specs are archived under `tasks/monthly-close-settlement/phase-3-settlement-preview/terminados/`. |
