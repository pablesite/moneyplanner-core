...
| Informe Fiscal Crypto | Media | 🔄 | MVP operativo en Core: Pionex + Binance, FIFO global cross-exchange y pantalla de informe. Phase 5 Pionex v2 completa (5A-5G): FX por minuto, fills bot en FIFO, matching venta→lotes, trazabilidad de syncs, export CSV/PDF, reliability gates API-first con CSV fallback auditado y bloqueo de resumen declarable. Binance queda para fase posterior. |
